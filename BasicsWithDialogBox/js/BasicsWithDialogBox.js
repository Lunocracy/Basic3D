class BasicsWithDialogBox {
  async run(env) {
    ...
    const boxWidth = Math.min(340, containerWidth - 30);
    const initialLeft = Math.max(16, containerWidth - boxWidth - 30);

    this.autoLoadedBox = UITools.makeDialog({
      env: this.env,
      title: 'Auto-Loaded Box',
      size: [boxWidth, 240],
      position: [initialLeft, 80],
    ...
    const defaultBoxOptions = {
      ...
      size: [280, 180],
      position: [Math.max(16, initialLeft - 40), 280],
      transparent: false,
